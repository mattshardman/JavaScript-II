...
// Problem 1
// randomly assign start groups
const grouping = (array, assignmentFunction) =>
  runners.map(each => ({
    ...each,
    group: assignmentFunction()
  }));
...
const groupedRunners = grouping(runners, randomGrouping);
console.log(groupedRunners);
// Problem 2
// announce how much the highest donators raised
const sortingHat = (array, sortFunction, reducerFunction) => {
  const sorted = sortFunction(array).slice(0, 5);
  const reduced = reducerFunction(sorted);
  return reduced;
};

const sortFunction = array => array.sort((a, b) => b.donation - a.donation);

const reducerFunction = array =>
  array.reduce((acc, each) => {
    acc.push(`Hi I am ${each.first_name} and I raised £${each.donation}!`);
    return acc;
  }, []);

const generous = sortingHat(runners, sortFunction, reducerFunction);
console.log(generous);

// Problem 3
// Prize: person with name beginning with 'S' and highest donation
const prizeWinner = runners
    .filter(each => each.first_name.charAt() === 'S')
    .sort((a, b) => b.donation - a.donation)[0];
